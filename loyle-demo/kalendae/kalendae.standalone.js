...
Kalendae.prototype = {
	...
	directions: {
		...
		'today-after-one' :function (date) {return moment(date).startOf('day').yearDay() < moment(date).endOf("day").yearDay();}
	},
	...
	weekSelected: function (mom) {
		...
		var end = moment(x).endOf('week').subtract('day',0);
		this._sel = [start, end];
	...
	draw : function draw() {
		...
		if (opts.directionScrolling) {
			...
			//只显示未来两个月
			 if (opts.direction === 'today-after-one') {
				if (diff > opts.months ) {
					this.disablePreviousMonth = false;
					util.removeClassName(this.container, classes.disablePreviousMonth);
				}
